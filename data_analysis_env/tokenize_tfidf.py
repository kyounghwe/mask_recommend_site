...
docs = pd.read_csv('mask_data.csv')
docs = docs.fillna('None')
docs['review_temp'] = docs['Name'] + docs['Category'] + docs['Mask_func']

vocab=[]
for i in range(len(docs)):
    temp = tokenizer.morphs(re.sub("[^ㄱ-ㅎ ㅏ-ㅣ 가-힣]", " ", "".join(docs['review_temp'][i]))) # 토큰화 -> morphs: 형태소 분석
    # print(i,temp)  # 각 행별로 형태소 분석이 어떻게 되었는지 확인 
...
docs_tfidf = pd.concat([docs, tfidf_], axis=1)

# 키워드에 따라서 tf-idf 점수 높은 순대로 정렬
keyword = ['가드', '황사']
docs_tfidf_ = docs_tfidf.sort_values(keyword, ascending=False)

print(docs_tfidf_.head())