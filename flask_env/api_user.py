...
def home():
    ...
    try:
        data = [
            ...
            [request.cookies.get('mask_function_1'), request.cookies.get('mask_function_2'), request.cookies.get('mask_function_3'), request.cookies.get('mask_function_4'), request.cookies.get(
                'mask_function_5'), request.cookies.get('mask_function_6'), request.cookies.get('mask_function_7'), request.cookies.get('mask_function_8')],
            request.cookies.get('mask_price'),
            request.cookies.get('recommend')
        ]
        mask_category = [data[0] if data[0] not in ['None', None] else ''][0]
        mask_blocking_grade = [data[1] if data[1]
    ...
    checked_list = [mask_category, mask_blocking_grade,
                    mask_function, mask_price, recommend]
    if checked_list[:-1] != ['', '', ['', '', '', '', '', '', '', ''], ['', '']]:
        ...
        if checked_list[-2] != ['', '']:
            if checked_list_for_html[-1] == 999999999:
                ...
                temp = " ~ ".join(
                    list(map(str, checked_list_for_html[-2:]))) + '원'
    ...
    # 카테고리 없고, 개인선호(추천)만 이용
    elif checked_list[:-1] == ['', '', ['', '', '', '', '', '', '', ''], ['', '']] and checked_list[-1] != '':
    ...
    if page != None:
        ...
        if session['page_num'] - page == 1 and session['page_num'] > 0:  # prev페이지로 갈 때
            session['page_num'] -= 1
        # next페이지로 갈 때
        elif session['page_num'] - page == -1 and session['page_num'] < len(mask_list)//16:
            session['page_num'] += 1
    else:
        session['page_num'] = 0
...
def setcookie():
    if request.method == "POST":
        ...
        try:
            resp.set_cookie('mask_category', request.form.get('mask_category'))
        except:
            resp.set_cookie('mask_category', 'None')
        try:
            resp.set_cookie('mask_blocking_grade',
                            request.form.get('mask_blocking_grade'))
        except:
            resp.set_cookie('mask_blocking_grade', 'None')
        try:
            resp.set_cookie('mask_function_1',
                            request.form.get('mask_function_1'))
        except:
            resp.set_cookie('mask_function_1', 'None')
        try:
            resp.set_cookie('mask_function_2',
                            request.form.get('mask_function_2'))
        except:
            resp.set_cookie('mask_function_2', 'None')
        try:
            resp.set_cookie('mask_function_3',
                            request.form.get('mask_function_3'))
        except:
            resp.set_cookie('mask_function_3', 'None')
        try:
            resp.set_cookie('mask_function_4',
                            request.form.get('mask_function_4'))
        except:
            resp.set_cookie('mask_function_4', 'None')
        try:
            resp.set_cookie('mask_function_5',
                            request.form.get('mask_function_5'))
        except:
            resp.set_cookie('mask_function_5', 'None')
        try:
            resp.set_cookie('mask_function_6',
                            request.form.get('mask_function_6'))
        except:
            resp.set_cookie('mask_function_6', 'None')
        try:
            resp.set_cookie('mask_function_7',
                            request.form.get('mask_function_7'))
        except:
            resp.set_cookie('mask_function_7', 'None')
        try:
            resp.set_cookie('mask_function_8',
                            request.form.get('mask_function_8'))
        except:
            resp.set_cookie('mask_function_8', 'None')
        try:
            resp.set_cookie('mask_price', request.form.get('mask_price'))
        except:
            resp.set_cookie('mask_price', 'None')
        try:
            resp.set_cookie('recommend', request.form.get('recommend'))
        except:
...
def join():
    if request.method == "POST":  # 회원가입 정보 기입 후 확인 -> 데이터베이스에 정보 저장
        ...
        member = tb_user_info(
            user_id, user_pw, user_gender, user_age, user_job)
...
def login():
    if request.method == 'GET':
        ...
        try:
            user = tb_user_info.query.filter(
                tb_user_info.user_id == user_id).all()  # 아이디 비교
            if user:  # 아이디 존재할 때
                user = tb_user_info.query.filter(
                    tb_user_info.user_pw == user_pw).first()  # 비밀번호 비교
